#!/usr/bin/env python3
"""Render six-model scaffold-diversity Table 1 and Figure 2 results."""
...
class ScaffoldResultStore(dense.ResultStore):
    def rows(self, kind: str, seed: int) -> list[dict]:
        ...
        selected = [row for row in rows if row.get("experiment") in relevant]
        if len(selected) != len(PANEL.models) * len(dense.MOLECULE_SWEEP):
            raise ValueError(
                f"Seed {seed} has {len(selected)} six-model scaffold rows; "
                f"expected {len(PANEL.models) * len(dense.MOLECULE_SWEEP)}"
            )
        mismatched = [
            row
            for row in selected
...
def _validate_manifest(results_root: Path) -> dict:
    ...
    if not isinstance(sources, list) or len(sources) != len(dense.SEEDS) * len(PANEL.models):
        raise ValueError("Manifest does not contain all 30 source files")
    if any(
        int(record.get("rows", -1)) != 10_000
        or len(str(record.get("rows_sha256", ""))) != 64
...
def _plot_figure2(store: ScaffoldResultStore, output_path: Path) -> None:
    ...
    figure, axis = plt.subplots(figsize=(12.8, 8.0))
    figure.patch.set_facecolor("white")
    dense._draw_main_panel(axis, store, PANEL)
    figure.supxlabel("Utility", y=0.205)
    figure.supylabel("Scaffold Diversity", x=0.025)
    figure.legend(
...
def _result_section(
    ...
    lines = [
        ...
        "",
        "| Sweep points | Runs | Samples per model and point | Total reused samples |",
        "|---|---:|---:|---:|",
        f"| `{molecule_points}` | 5 | 1,000 | 300,000 |",
        "",
...
def main() -> None:
    ...
    store = ScaffoldResultStore(args.results_root, ("denovo",))
    ...
    figure_path = output_dir / "figure2-scaffold-diversity-six-model.pdf"
    standalone_path = output_dir / "scaffold-diversity-six-model-results.md"
    ...
    standalone_path.write_text(
        "# Six-Model Scaffold Diversity Results\n\n" + section
    )
